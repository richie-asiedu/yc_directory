    ...
    <>
      ...
      <div className="w-full bg-[#EE2B69] min-h-[380px] mb-8 mt-[3%] relative overflow-hidden">
        ...
        <div className="flex justify-center w-full">
          <p className="text-white text-[20px] text-center mt-4 w-full max-w-[60%]">{card.desc}</p>
        </div>
      </div>
      ...
      <main className="min-h-screen">
        <section className="max-w-4xl mx-auto mt-10">
          ...
          <div className="mt-8 flex items-center gap-4">
            <Image src={card.avatar} alt={card.author} width={48} height={48} className="rounded-full" />
            <div>
              <div className="font-extrabold text-[30px] font-sans">{card.author}</div>
              <div className="text-black text-[20px] font-sans">{card.mail}</div>
            </div>
          ...
          <div className="mb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 font-sans">
              {defaultStartupCards.filter((c: { id: number }) => c.id !== card.id).slice(0, 3).map((similar: any) => (
                <div key={similar.id} className="bg-white rounded-[2rem] shadow-6xl p-6 flex flex-col h-full relative border-8 border-black transition-colors duration-200 hover:border-[#EE2B69]" style={{ minHeight: 420 }}>
                  <div className="flex justify-between items-center mb-5 border-black">
                    <span className="bg-pink-100 text-black text-base font-semibold px-3 py-2 rounded-full">{(() => {
                      const dateField = (similar as any)._createdAt || (similar as any).createdAt || (similar as any).date || (similar as any).created || (similar as any).publishedAt;
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <div>
                      <div className="text-base font-semibold text-black leading-tight mb-4">{similar.author}</div>
                      <div className="text-[26px] font-extrabold text-black leading-tight font-sans">{similar.title}</div>
                    </div>
                    <Image src={similar.avatar} alt="avatar" width={48} height={48} className="w-12 h-12 rounded-full object-cover border-2 border-white ml-2" priority />
                  </div>
                  <div className="text-[#333333] text-[16px] mb-3 leading-snug line-clamp-2 font-sans">{similar.desc}</div>
